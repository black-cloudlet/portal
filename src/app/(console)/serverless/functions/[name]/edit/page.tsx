...
export default async function EditFunctionPage({ params }: { params: Promise<{ name: string }> }) {
  const { name } = await params;
  ...
  try {
    [info, wl] = await Promise.all([
      getPlatformInfo(),
      getWorkload("function", activeGroup, name, accessToken),
    ]);
  } catch (err) {
    return <ApiErrorNotice error={err} />;
  }
  return <WorkloadForm mode="edit" type="function" info={info} group={activeGroup} initial={wl} />;
}
